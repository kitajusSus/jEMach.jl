...
local function fd_set_new()
  ...
  for i = 0, 15 do s.fds_bits[i] = 0 end
  return s
end
...
local function load_state()
  ...
  if mt == last_mtime then return end
  last_mtime = mt
...
local function clamp_cursor()
  ...
  if cursor > #display_rows then cursor = #display_rows end
end
...
local function render(term_rows, term_cols)
  ...
  -- Title bar
  local title = " Julia Workspace  [" .. repl_pane .. "]"
...
local function main()
  ...
  local ok, err = pcall(function()
    ...
    while true do
      ...
      -- Read input with timeout so we can refresh state periodically
